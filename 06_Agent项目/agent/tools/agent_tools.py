...
from rag import RagSummarizeService, VectorStoreService
from langchain_core.tools import tool
from utils import agents_config, get_abs_path, logger
...
def generate_external_data() -> None:
    if not external_data:
        if "external_data_path" not in agents_config:
...
@tool(description="无入参，无返回值，调用后触发中间件自动喂报告生成场景动态注入上下文信息，为后续提示词切换提供上下文支撑")
